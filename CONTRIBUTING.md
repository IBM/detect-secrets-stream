...
After a new secret detector has been developed in `IBM/detect-secrets` (see [instructions](https://github.com/IBM/detect-secrets/blob/master/CONTRIBUTING.md)), additional functionality must be developed in this repo in order to support the new secret type as part of our server side detection pipeline. This involves creating a validator class containing the functions `secret_type_name()`, `verify()`, `resolve_owner()` and `revoke()`, at a minumum.
