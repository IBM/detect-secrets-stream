...
The Admin Tool is a companion app for `detect-secrets-stream` to allow it to scan private / internal repositories and add security focals. This documentation is intended for GitHub Enterprise organization admins.

<!-- START doctoc generated TOC please keep comment here to allow auto update -->
<!-- DON'T EDIT THIS SECTION, INSTEAD RE-RUN doctoc TO UPDATE -->

- [As a GitHub organization admin, why have I been asked to install the Detect Secrets Admin Tool GitHub App and/or review a PR in `dss-config`?](#as-a-github-organization-admin-why-have-i-been-asked-to-install-the-detect-secrets-admin-tool-github-app-andor-review-a-pr-in-dss-config)
...
- [What does it mean if I can only request to install the Detect Secrets Admin Tool on a repository or organization?](#what-does-it-mean-if-i-can-only-request-to-install-the-detect-secrets-admin-tool-on-a-repository-or-organization)
...
If you're a GitHub organization admin who's been asked to install the Detect Secrets Admin Tool GitHub to your organization and/or approve a PR in `dss-config`, this means that security personnel (security focals) associated with your GitHub organization have found it necessary to monitor for secret leaks within your organization via the GitHub app.
...
-   **Associate security focals to organizations**, so the security focals can be subscribed to token leak communication, such as ServiceNow and email notifications.
...
When reviewing PRs in `dss-config`, it is important to verify that all the `security-focal-emails` listed in the PR's config file (in `dss-config/org_set_config/<filename>.yaml`) _should_ have access to Service Now tickets regarding secret leaks in your org. You should also ensure that your organization appears under `organizations` in that file, otherwise you should not need to review the PR.
...
## What does it mean if I can only request to install the Detect Secrets Admin Tool on a repository or organization?
