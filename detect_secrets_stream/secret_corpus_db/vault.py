...
from ibm_secrets_manager_sdk.secrets_manager_v2 import *  # noqa: F401,F403
...
class Vault(BaseVaultBackend):
    def __init__(self):
        ...
        if os.path.exists(config_file):
            ...
            # begin-common
            secrets_manager_service = SecretsManagerV2.new_instance()  # noqa: F405
            # end-common
            assert secrets_manager_service is not None

            # Load the configuration
            global config
            config = read_external_sources(SecretsManagerV2.DEFAULT_SERVICE_NAME)  # noqa: F405
    ...
    def create_or_update_secret(self, token_id: int, secret: str, other_factors=None):
        ...
        if read_status == 200:
            ...
            try:
                ...
                raise VaultReadException(
                    'Error updating secret to Secrets Manager. Something is wrong',
                )
            ...
            try:
                ...
                raise VaultReadException(
                    'Error writing secret to Secrets Manager. Something is wrong',
                )
    ...
    def read_secret(self, token_id: int):
        ...
        try:
            ...
            raise VaultReadException(
                'Error reading secret from Secrets Manager. Secret might not be in Secrets Manager.',
            )
        else:
            return secret['data']
