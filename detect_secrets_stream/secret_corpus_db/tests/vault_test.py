...
class VaultTest(TestCase):
    ...
    def setUp(self, mock_load_vault_conf, mock_hvac):
        ...
        source_file = open(vault_conf_path, 'w')
        mock_vault_conf = (
            'SECRETS_MANAGER_URL=https://instance.us-east.secrets-manager.appdomain.cloud\n'
            'SECRETS_MANAGER_AUTH_TYPE=iam\nSECRETS_MANAGER_APIKEY=key'
        )
        print(mock_vault_conf, file=source_file)
        source_file.close()
    ...
    def test_read_secret(self):
        ...
        self.assertEqual(
            data,
            {'secret': 'super_secret', 'other_factors': {'another': 'one'}},
        )
